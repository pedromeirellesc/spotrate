<x-modal name="{{ $name }}" title="{{ $title }}" maxWidth="3xl" :show="$errors->any()">
    ...
    <form method="POST" action="{{ route('reviews.store') }}" class="space-y-4">
        ...
        <div x-data="ratingComponent()" class="flex space-x-1 mt-1">
            <template x-for="star in 5" :key="star">
                <button type="button" @click="setRating(star)" @mouseover="hover = star" @mouseleave="hover = 0"
                    class="focus:outline-none">
                    <svg x-bind:class="(star <= (hover || rating)) ? 'text-yellow-400' : 'text-gray-300'"
        ...
        <div>
            ...
            <x-textarea id="comment" class="block mt-1 w-full" name="comment">{{ old('comment') }}</x-textarea>
